...
export default {
   ...
   components:{
    ...
    methods: {
        ...
        this.form.solution_id = selected
        if(selected.length>0){
            ...
            confirmButtonText: 'Xoá!'
                ...
                if(Permissions.indexOf('delete solutions') == -1){
                    ...
                    if (result.value) {
                        this.form.post(route('multiple.solutions'))
                        .then(response => {
                            if(response.data.status == 'success'){
                                this.getData(route("users"), this.tableProps)
                                Swal.fire(
                                    'Đã xoá',
                                    'Xoá giải pháp thành công!',
                                    'success'
                                );
                            }
                        }).catch(()=>{
            ...
            Swal.fire({
                title: 'Chưa chọn giải pháp',
                text: "Chưa có giải pháp nào được chọn!",
                icon: 'warning',
            });
        }
      }
